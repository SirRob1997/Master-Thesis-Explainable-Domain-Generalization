...
The occurring minimization problem is then often solved through iterative gradient based optimization algorithms \eg \sgd{} \citep{Robbins1951} or \adam \citep{Kingma2015} which perform reasonably well for the non-convex, continuous loss surfaces produced by modern machine learning problems and architectures.
...
\begin{table}[htbp]
    ...
    \toprule
    \textbf{Dataset}  & \textbf{Reference} & $\env^1$   &  $\env^2$ & $\env^3$ & $\env^4$ & $\env^5$ & $\env^6$ & $\dots$ & $\env^{75}$ \\
    \midrule
