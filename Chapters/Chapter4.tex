...
To improve upon the domain-agnostic prototype layer, we analyze the best performing hyperparameters (oracle-validation) for their learned, pairwise prototype $\ell_2$-distance as well as the cosine distance $\cdistance$ which for any two prototypes $\proti_i$ and $\prot$ are given by:
\begin{alignat}{3}
...
In this section, we are going to look at different ablations of the presented methods and how the individual components impact the performance. In particular, for \divcams, we analyze different methods of resetting the masks (mask batching) in \Cref{sec:ablation_study_batching} and see how additional methods that are supposed to improve the underlying class activation maps impact performance in \Cref{sec:ablation_study_batching}. On top of that, for \prodrop, we evaluate the effect of self-challenging for different negative weights in \Cref{sec:abl_self_challenging}, as well as the impact of the additional intra-loss factor in \Cref{sec:intra_loss}.
